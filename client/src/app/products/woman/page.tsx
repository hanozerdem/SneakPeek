...
const Page = () => {
  ...
  const filteredProducts = products
    .filter((p) => p.category === "Woman" || p.category === "Unisex")
    .filter((product) => {
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    const aPrice = a.prices?.[0]?.price ?? a.price ?? 0;
    const bPrice = b.prices?.[0]?.price ?? b.price ?? 0;
  ...
  return (
    <>
      <div className="pt-24 px-4 md:px-8">
        <h1 className="px-2 text-4xl md:text-6xl font-bold mb-4">WOMEN</h1>
        ...
        <div className="mt-8 flex justify-between items-center gap-4">
          ...
          <button
            ...
            onClick={() => {
              setSortOption("");
              setFilters({ tag: "", category: "Woman", price: "", color: "" });
            }}
          >
            Clear all filters
          </button>
